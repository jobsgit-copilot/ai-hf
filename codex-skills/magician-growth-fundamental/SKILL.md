...
### 0. 量化硬规则（P1 回测校准，2026-08，`reports/magician-vcp-funnel-20260811.md`）

以下条件为**一票否决**（按财报披露日 point-in-time，数据缺失不否决）：

| 规则 | 阈值 | 回测依据 |
|------|------|---------|
| 质量红线 | 非ST/未退市；负债率≤85%；经营现金流/净利润中位数≥0.7；毛利率均值≥15%；年报ROE均值≥8% | 通过率 40%；F0→F1 期望 +0.8~2.3pp |
| 营收成长 | 最新季度营收同比 ≥15%（与去年同季已披露值比） | 通过率 28%；F1→F2 期望再 +1~2pp，中位数转正 |
| 连续两季增长（v8 稳健版，可选） | 营收与归母净利润连续两季同比均>0（累计口径，point-in-time） | 样本外 C/D 2.17→4.27、回撤 -11.2→-5.6%（`--funnel F6F1`）；弱市防御型，默认不启用 |
| 营收加速 | **不作硬性否决**（通过率仅3%样本枯竭）；改为观察：同比≥15% 且较上季回落 <10pp | 硬性加速弃用 |
| PB 估值 | 自身历史分位≤90 不作硬否决；仅量能萎缩+RS≥90 时作加分 | F4 效果混杂 |
