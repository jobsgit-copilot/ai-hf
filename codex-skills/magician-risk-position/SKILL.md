...
- **回测校准（2026-08-12）**：固定 RR3 目标下 50/80 几乎不触发；保本 0.0 是陷阱（胜率 4.7%）、0.20 训练窗有效但样本外失效——不建议作为收益优化，仅作心理/风险工具。
...
- **风险 2.0% 仅作进取版（P2 扫描+样本外）**：训练窗 CAGR 5.9→7.6% 但回撤 -18.2→-23.7%（C/D 持平 0.32）；样本外 18.4%/-13.2%/C/D 1.39 < v6 的 1.60。追求绝对收益可用，默认维持 1.5%。
