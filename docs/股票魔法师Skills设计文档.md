...
- **第二梯队验证与优化（①②③⑤，2026-08-12）**：⑤ **RS 优先入场采纳为 v7**（`--rs-priority`，同日多信号按 RS 降序、领头羊优先）——全样本 C/D 0.55→0.86、测试窗 1.62→2.16，0.5% 摩擦后仍 0.71；① 摩擦成本 0.2%/0.5% 使 v6 CAGR -0.8/-1.9pp（实盘按 0.3-0.5% 打折，v7 约 12-13%）；② 连续分段与独立 OOS 一致（边界效应可忽略）；③ risk 1.25-1.75 为平坦高原（C/D 全 0.55）、max_ext 0.18 非单调判噪声。工具新增 `--friction/--rs-priority/--window-stats`；⑥（收紧方向）放弃。

- **④ 50/80 盈利保护验证（2026-08-12，不采纳）**：固定 RR3 目标（+21%）先于 +50% 峰值触发，trail80 几乎永不激活；无目标后策略失效（-0.3%）；保本 0.20 训练窗 C/D 0.51→0.66 但邻域（0.10/0.30）很差、样本外更差（2.14→1.84）——训练窗尖峰，不采纳；保本 0.0 是假象（胜率 4.7%）。结论：**维持 v7**（固定目标+RS优先），50/80/保本仅作心理/风险工具。工具新增 `--protect-be/--trail80/--no-target`（已实现并测试，默认关闭）。



## 8.3 第三梯队：净利润成长 + 板块联动（2026-08-12）

在 v7 机制（F1 + 收缩≥3 + dy分级 + RS优先 + 固定RR3，锁仓80/6仓/风险1.5%）之上，补齐净利润数据并引入板块联动，验证“成长加强 + 板块共振”能否进一步改善组合。本轮按要求**不考虑退市与次新过滤**（ST 仍由 r_st 排除）。报告：`reports/magician-vcp-tier3-20260812.md`。

- **净利润 point-in-time 补库**：`finance_income.pkl` 扩展为含 `n_income_attr_p/n_income/basic_eps`（154,415 行、5534 只、2019→2026-08）；新增 `tools/build_netprofit_db.py`（tushare 逐股拉取，在 ai2miniqmt 虚拟环境运行约 52 分钟）。`magician_fundamental.py` 的 `snapshot()` 新增 `np_yoy/np_yoy_prev`（按 `ann_date` 严格 point-in-time，与营收同比同口径），新增 `g_np` 规则（默认阈值 25%，`--np-min` 可覆盖）与漏斗分级 `F2N`（质量+净利）/ `F5`（营收+净利双成长）。
- **板块联动模块**：新增 `tools/magician_sector.py`，基于申万一级行业（`index_member.pkl` point-in-time 归属 + `industry_index_daily.pkl` 行业指数），对全量事件附加 `industry`（96.3%）、`sector_rs`（行业 126 日涨幅分位，95.9%）、`sector_n_vcp_20d`（同行业近 20 日 VCP 标的数，即板块共振）；数据缺失按“缺失不否决”处理。回测/组合引擎新增 `sector_rs_min/sector_res_min` 过滤参数。
- **净利润过滤不划算（不采纳为硬过滤）**：F2N 净利≥25 训练窗 C/D 0.51→0.06、样本外 2.17→1.36，交易数腰斩（159→57）；F5 双成长训练窗 C/D 0.09，比 F2 单营收更差；15/20/25/30 阈值单调无拐点，说明问题在信号覆盖而非阈值。质量红线 F1 已是当前样本的最优平衡点。
- **板块共振训练窗提升、样本外略降**：F1+板块共振≥3 训练窗 C/D 0.51→0.62（CAGR 8.6→10.0%、回撤 -17.1→-15.9%），样本外 C/D 2.17→1.94、CAGR 24.4→18.8%；F1+板块RS≥70 训练 0.60、样本外 1.62，弱于共振口径。
- **结论**：**无变体超过 v7**，板块共振不进入硬过滤；建议 F1 + 板块共振≥3 作为**可选观察/加仓参考**（用于信号排序与二次确认），F2N/F5 留作信号充裕时（如 2024-2026 样本外）的人工复核维度。本轮跳过退市/次新过滤的影响已在报告注明（样本外 F2 对照 C/D 2.67 明显偏高，需在后续轮次单独评估）。



## 8.4 第四梯队：仅双成长连续两季（忽略 F1 质量红线）（2026-08-12）

按要求验证“只保留营收+归母净利润连续两季同比正增长、其余 F1 质量红线全部忽略、其他条件不变（v7 机制）”的组合表现。报告：`reports/magician-vcp-tier4-20260812.md`。

- **期间修复一个数据口径 bug**：原 `snapshot()` 用“最新报表期−91/365 天”推“上一季/去年同期”，年报期会错位、闰年交界漂移 1 天，导致 `rev_yoy_prev/np_yoy_prev` 几乎全缺失（F6 首跑 0 笔交易）。改为按每只股票实际报表期建“上一期/去年同期”映射（去年同期按同月同日匹配），营收/净利同比字段覆盖率提升到 93%/80%；该修复只提升数据覆盖，F1 基线回测完全复现（C/D 0.51/2.17）。
- **只留双成长、丢掉质量红线不可行**：F6（连续两季>0，忽略全部 F1 含 ST）训练窗 C/D 0.51→0.08（CAGR 8.6%→1.4%、期望 1.70%→−0.04%），样本外 C/D 2.17→2.12、CAGR 24.4%→14.0%；F6B（连续两季均≥15%）训练窗转负（C/D −0.13），样本外 C/D 2.51 属“样本外尖峰”（暴露仅 23%），不采纳；F6C（仅当季>0、不要求连续）训练窗 0.19，说明“连续两季”约束本身不是损失主因。
- **归因**：F1 质量红线（负债率/现金流利润比/毛利率/ROE）的贡献主要在训练窗——它削掉左尾（期望 +1.70%→−0.04%），是 2020-2023 组合不亏钱的关键；营收/净利成长更像“放大器”而非“守门员”，只做成长不做质量会把脆弱标的放进组合。
- **结论**：维持 v7（F1 质量红线 + VCP + dy分级 + RS优先 + 固定RR3）。双成长连续两季（F6/F6B）仅作可选观察/信号排序加分维度，不进硬过滤。新增漏斗级别 F6/F6R/F6B/F6C 与扫描脚本 `tools/magician_tier4.py`；F6R=F6（ST 排除对已成交事件无影响，ST 标的本就难通过其余条件）。



## 8.5 第四梯队(续)：F1 + 双成长连续两季叠加（2026-08-12）

在 F1 质量红线之上叠加“营收+净利连续两季增长”，并对比累计口径与单季口径。报告：`reports/magician-vcp-tier4-sq-20260812.md`。

- **单季口径实现**：`snapshot()` 新增 `sq_rev_yoy/sq_rev_yoy_prev/sq_np_yoy/sq_np_yoy_prev`（单季值 = 该期累计 − 同财年上一期累计，一季报即本身，再做同比），规则 `g_sqdual2/g_sqdual2_15`，漏斗新增 `F7`（F1+单季双成长2Q>0）、`F7B`（≥15%）、`F6F1`（F1+累计双成长2Q>0，口径对照）。
- **F7（用户指定，单季口径）不采纳**：训练 C/D 0.51→0.33、交易 159→66，样本外 CAGR 24.4%→15.6%（C/D 2.17→2.12）；逐年 C/D 1.57/1.76/6.94 不稳定，2024-2025 明显弱于基线。F7B（≥15%）训练 0.34、样本外 0.88，覆盖过紧更差。
- **F6F1（累计口径叠加）为意外亮点**：训练 C/D 0.46（仅略降）、样本外 C/D 2.17→**4.27**、最大回撤 −11.2%→−5.6%、CAGR 基本持平（24.4%→24.1%）、交易 104→92。样本外逐年 C/D 3.47/4.95/7.61 全面高于基线 3.10/3.17/2.84，且回撤逐年减半——不是单年尖峰。结合 8.4：纯双成长（无 F1）训练 0.08，说明“质量红线守门 + 双成长确认”组合才有效。
- **结论**：单季口径叠加不采纳；**F6F1 列为 v8 候选**，建议先补验证：阈值邻域（0/5/10）、0.3-0.5% 摩擦成本、更长历史（含 2018-2019）与实盘约束（涨跌停成交）再决定入库。



## 8.6 v8 候选（F6F1）入库评审：Step1 邻域 / Step2 成本 / Step3 历史（2026-08-13）

按 8.5 定义的评审闸门对 F6F1（F1 + 累计口径双成长连续两季>0）执行五步中的 Step1-3。报告：`reports/magician-vcp-v8-gate-20260813.md`。工具新增：`apply_rules` 支持 `dual2_min` 阈值参数（`>0` 语义不变）、漏斗 `F6F1C`（上一季开关关）、`simulate(limit_exec=True)` 一字涨跌停成交约束、扫描脚本 `tools/magician_tier4_v8.py --step all/step1/step2/step3`。

- **Step1 邻域——通过（缓坡非尖峰）**：阈值 0/5/10/15 样本外 C/D 4.27/3.15/2.20/1.95、训练 0.46/0.45/0.58/0.34；th=0 最优但衰减渐进，th=5 仍 3.15≫基线 2.17，非单点尖峰；F6F1C（仅当季）OOS 2.87 < 4.27，确认“连续两季”要求有增量。
- **Step2 成本执行——通过**：0.5% 摩擦后 F6F1 OOS C/D 4.27→3.86（F1 2.17→1.90），优势保持；一字涨跌停成交约束 0 笔受影响（VCP 突破为放量非一字）；代价为训练窗暴露仅 18%（F1 33%）。
- **Step3 历史结构——通过（弱市防御型）**：按年 C/D 7 年中 6 年 ≥ F1，唯一大幅落后是 2020（0.62 vs 3.47，大涨年少参与）；up 段 C/D 持平、down 段全面占优（0.78 vs 0.50、回撤 −7.8 vs −12.5）；关 rs_priority/nody_half 后 OOS 降至 2.71/3.16，均保持正贡献，非抵消机制。
- **总体判定：有条件通过，不建议直接替换默认**。入库方式三选一：① 拒绝维持 v7；② **作为“稳健/防御版”与 v7 并列入库（推荐）**，`--funnel F6F1` 供弱市/低回撤偏好使用，默认仍 v7；③ 环境自适应组合（up 用 F1、down 用 F6F1），需先跑组合验证整体 C/D 与回撤再决定是否升级默认。等待用户拍板入库方式。



## 8.7 v8 入库决策：以方式 2 与 v7 并列（2026-08-13）

按用户决定，F6F1（v8 稳健版）**与 v7 并列入库，默认仍 v7**，不作为默认替换。

- **入库动作**：`skills/magician-sepa.md` 参数校准表新增"基本面漏斗（v8 稳健版，可选）"行与 v8 段落；`skills/magician-growth-fundamental.md` 量化硬规则表新增"连续两季增长（v8 稳健版，可选）"行；`scripts/sync-codex-skills.py` 已同步 `codex-skills/*`。
- **工具能力**：`magician_portfolio.py --funnel` 可选值扩展至 F6F1/F6F1C/F7/F7B/F2N/F5/F6 等；新增 `--rule-dual2-min`（累计双成长阈值，默认 0 表示>0）；`--limit-exec` 模拟一字涨跌停成交约束。
- **用法**：弱市/低回撤偏好 → `--funnel F6F1`（样本外 C/D 4.27、回撤 −5.6%）；默认策略保持 `--funnel F1`（v7）。
- 评审全程见 `reports/magician-vcp-v8-gate-20260813.md`；环境自适应组合（up F1 / down F6F1）留作后续可选优化。
