...
- **满仓敏感性（2026-08-12 补测）**：基线平均现金 81%（F1mc3）/69%（F1mc2），45% 采样周完全空仓；“全部满仓”（`--full-invest`，新仓尽量用到单只上限）仅把暴露提到 27%（单只25%cap，CAGR 9.5%、回撤 -34.4%）或 36%（单只50%cap，CAGR 14.1%、回撤 -41.1%）——空仓周仍占 ~46-50%，**瓶颈是信号供给而非仓位上限**，满仓放大回撤、CAGR/回撤 从 0.48 降到 0.19-0.34；提升收益应扩大信号供给而非加满仓位。详见 `reports/magician-vcp-portfolio-20260812.md` 附录。

- **第一梯队放宽扫描（2026-08-12，组合层单变量）**：量能萎缩从“必选”改为**分级**（末段≤首段60% 全仓 / 未达标半仓，`--nody-half`）是唯一“放宽且更优”的方向——暴露 19→35%、空仓周 45→5%、CAGR 9.0→10.0%、回撤 -18.9→-18.2%，C/D 0.48→0.55（全研究最高）；dy 全关 CAGR 11.5% 但回撤 -25.6%（收益换回撤）；mc2 混仓只改善 mc2 回撤（C/D 0.28→0.33）；锁仓 60 无贡献；持仓 8 只反而更差（C/D 0.31，满 6 仓是隐性质保）。推荐参数升级 v6：F1 + 收缩≥3 + dy 分级 + 锁仓80/6仓/风险1.5%。详见 `reports/magician-vcp-portfolio-20260812.md` 附录。

- **第二梯队扫描 + 样本外验证（2026-08-12）**：训练窗（2020-2023）8 个放宽变体（horizon 45/90、risk2.0、max_ext 0.20、环境系数 down×0.7、ROE≥6、OCF≥0.5）**无一超过 v6 的 C/D（0.33）**，v7=维持 v6；risk2.0 为“进取版”（CAGR +1.7pp/回撤 +5.5pp，C/D 持平）。样本外（2024-2026）v6 CAGR 16.1%、回撤 -10.1%、C/D 1.60，三年全正，7 年中 6 年正（唯一负年 2023 -2%）——**v6 未过拟合**；horizon90 训练窗看似无害、样本外崩到 C/D 0.61，被样本外排除。工具新增 `--max-ext/--regime-down/--start/--end/--rule-*`。详见 `reports/magician-vcp-portfolio-20260812.md` 附录。
